...
export function FollowUpDetailsModal({
  ...
  const [currentLightboxPhoto, setCurrentLightboxPhoto] = useState<string>('');
  ...
  // Build photos array when followUp data changes
  useEffect(() => {
    if (followUp) {
      const photos: string[] = [];

      // Add check-in photo
      if (followUp.siteInPhotoUrl) {
        photos.push(followUp.siteInPhotoUrl);
      }

      // Add site photos
      if (followUp.sitePhotos && Array.isArray(followUp.sitePhotos)) {
        followUp.sitePhotos.forEach((photo: any) => {
      ...
      // Add check-out photo
      if (followUp.siteOutPhotoUrl) {
        photos.push(followUp.siteOutPhotoUrl);
      }

      // Add checkout site photos
      if (followUp.siteOutPhotos && Array.isArray(followUp.siteOutPhotos)) {
        followUp.siteOutPhotos.forEach((photo: any) => {
          const photoUrl = typeof photo === 'string' ? photo : photo.url;
          if (photoUrl) photos.push(photoUrl);
        });
      }
  ...
  const openLightbox = useCallback((photoUrl: string) => {
    ...
    setCurrentLightboxPhoto(photoUrl);
  ...
  const navigateLightbox = useCallback((direction: 'next' | 'prev') => {
    ...
    setCurrentLightboxPhoto(allPhotos[newIndex]);
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowLeft') {
        ...
        setCurrentPhotoIndex((prevIndex) => {
          const newIndex = (prevIndex - 1 + allPhotos.length) % allPhotos.length;
          setCurrentLightboxPhoto(allPhotos[newIndex]);
          return newIndex;
        });
      }
  ...
  }, [isLightboxOpen, allPhotos]);

  // Prevent main dialog close when lightbox is open
    ...
    <>
                      ...
                      <div className="relative group max-w-full sm:max-w-md">
                        <div className="relative overflow-hidden rounded-xl">
                          <img
                            ...
                            onClick={() => openLightbox(followUp.siteInPhotoUrl)}
                          />
                      ...
                      <div className="relative group max-w-full sm:max-w-md">
                        <div className="relative overflow-hidden rounded-xl">
                          <img
                            ...
                            className="w-full h-56 sm:h-64 object-cover border-2 border-gray-200 shadow-md transition-all duration-300 hover:shadow-xl hover:scale-[1.02] cursor-pointer"
                            onClick={() => openLightbox(followUp.siteOutPhotoUrl)}
                          />
      ...
      {isLightboxOpen && (
        ...
        >
            ...
            <>
              <button
                onClick={(e) => {
                  ...
                  setCurrentPhotoIndex((prevIndex) => {
                    const newIndex = (prevIndex - 1 + allPhotos.length) % allPhotos.length;
                    console.log('New index:', newIndex, 'Photo:', allPhotos[newIndex]);
                    setCurrentLightboxPhoto(allPhotos[newIndex]);
                    return newIndex;
                  });
                }}
                className="absolute left-4 z-[10000] p-3 rounded-full bg-white/10 hover:bg-white/20 transition-colors"
              >
              ...
              <button
                ...
                className="absolute right-4 z-[10000] p-3 rounded-full bg-white/10 hover:bg-white/20 transition-colors"
              >
          ...
          {/* Photo Counter */}
          {allPhotos.length > 1 && (
            <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-[10000] px-4 py-2 bg-white/10 rounded-full text-white text-sm">
              {currentPhotoIndex + 1} / {allPhotos.length}
            </div>
          )}
          ...
          {/* Keyboard Hint */}
          <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-[10000] px-4 py-2 bg-white/10 rounded-full text-white text-xs">
