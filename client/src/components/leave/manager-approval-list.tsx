...
import { CheckCircle, XCircle, Clock, Calendar, Loader2, User } from "lucide-react";
...
export function ManagerApprovalList() {
  ...
  const { data: pendingLeaves, isLoading } = useQuery<any[]>({
    queryKey: ["/api/leave-applications/pending-manager"],
    enabled: !!user,
  });

  const approveMutation = useMutation({
    mutationFn: async ({ leaveId, remarks }: { leaveId: string; remarks?: string }) => {
      return apiRequest(`/api/leave-applications/${leaveId}/approve-manager`, "PUT", { remarks });
    },
    onSuccess: () => {
      toast({
        title: "Leave Approved",
        description: "Leave application has been forwarded to HR for final approval.",
      });
      queryClient.invalidateQueries({ queryKey: ["/api/leave-applications/pending-manager"] });
        ...
        <DialogContent data-testid="dialog-approval">
          ...
          <DialogFooter>
            ...
            >
              ...
              {approvalAction === "approve" ? "Approve & Forward" : "Confirm Rejection"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
