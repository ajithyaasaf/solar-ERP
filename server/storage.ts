...
export interface IStorage {
  ...
  // Enhanced quotation system methods
  // Quotation Drafts
  ...
  updateQuotationDraft(id: string, data: Partial<InsertQuotationDraft>): Promise<QuotationDraft>;
  deleteQuotationDraft(id: string): Promise<void>;

  // Finalized Quotations
  listEnhancedQuotations(filters?: {
    status?: string;
    customerId?: string;
    projectStatus?: string;
  }): Promise<QuotationEnhanced[]>;
  getEnhancedQuotation(id: string): Promise<QuotationEnhanced | undefined>;
  createEnhancedQuotation(data: InsertQuotation): Promise<QuotationEnhanced>;
  updateEnhancedQuotation(id: string, data: Partial<InsertQuotation>): Promise<QuotationEnhanced>;
  finalizeQuotationDraft(draftId: string, quotationNumber: string): Promise<QuotationEnhanced>;

  // Quotation Documents
  getQuotationDocuments(quotationId: string): Promise<QuotationDocument[]>;
  ...
  getQuotationCommunications(quotationId: string): Promise<QuotationCommunication[]>;
  createQuotationCommunication(data: z.infer<typeof quotationCommunicationSchema>): Promise<QuotationCommunication>;

  // Pricing History
  getQuotationPricingHistory(quotationId: string): Promise<PricingHistory[]>;
  createPricingHistoryEntry(data: z.infer<typeof pricingHistorySchema>): Promise<PricingHistory>;
  ...
  updateQuotationTemplate(id: string, data: Partial<z.infer<typeof quotationTemplateSchema>>): Promise<QuotationTemplate>;

  // Site Visit Integration Helper Methods
...
export class FirestoreStorage implements IStorage {
  ...
  // ===== ENHANCED QUOTATION DRAFT MANAGEMENT METHODS =====

  async listQuotationDrafts(filters?: { 
    status?: string; 
    projectType?: string; 
    customerId?: string; 
    createdBy?: string;
    ...
    try {
      let query = this.db.collection("quotation_drafts") as Query;
      ...
      if (filters?.siteVisitId) {
        query = query.where("siteVisitId", "==", filters.siteVisitId);
      }
      ...
      return snapshot.docs.map(doc => {
        ...
        return {
          ...
          createdAt: data.createdAt?.toDate() || new Date(),
          updatedAt: data.updatedAt?.toDate() || new Date(),
          sourceVisitDate: data.sourceVisitDate?.toDate() || new Date(),
          followUpSchedule: data.followUpSchedule?.map((date: any) => date.toDate ? date.toDate() : new Date(date)) || [],
          approvedAt: data.approvedAt?.toDate(),
        } as QuotationDraft;
      });
    } catch (error) {
      console.error("Error listing quotation drafts:", error);
      return [];
    }
  }

  async getQuotationDraft(id: string): Promise<QuotationDraft | undefined> {
    try {
      const doc = await this.db.collection("quotation_drafts").doc(id).get();
      if (!doc.exists) return undefined;
      ...
      console.error("Error getting quotation draft:", error);
      return undefined;
    }
  }

  async getQuotationDraftBySiteVisit(siteVisitId: string): Promise<QuotationDraft | undefined> {
    try {
      const snapshot = await this.db.collection("quotation_drafts")
      ...
      console.error("Error getting quotation draft by site visit:", error);
      return undefined;
    }
  }

  async getQuotationDraftsByCustomer(customerId: string): Promise<QuotationDraft[]> {
    try {
      const snapshot = await this.db.collection("quotation_drafts")
      ...
      console.error("Error getting quotation drafts by customer:", error);
      return [];
    }
  }

  async createQuotationDraft(data: InsertQuotationDraft): Promise<QuotationDraft> {
    try {
      const validatedData = insertQuotationDraftSchema.parse(data);
      const id = this.db.collection("quotation_drafts").doc().id;
      const now = new Date();
      ...
      const draftData = {
        ...
        createdAt: Timestamp.fromDate(now),
        updatedAt: Timestamp.fromDate(now),
        sourceVisitDate: Timestamp.fromDate(validatedData.sourceVisitDate),
        followUpSchedule: validatedData.followUpSchedule.map(date => Timestamp.fromDate(date)),
        approvedAt: validatedData.approvedAt ? Timestamp.fromDate(validatedData.approvedAt) : null,
      };

      await this.db.collection("quotation_drafts").doc(id).set(draftData);
  ...
  async updateQuotationDraft(id: string, data: Partial<InsertQuotationDraft>): Promise<QuotationDraft> {
    try {
      const updateData: any = {
        ...data,
        updatedAt: Timestamp.fromDate(new Date()),
      ...
      // Handle date fields
      if (data.sourceVisitDate) {
        updateData.sourceVisitDate = Timestamp.fromDate(data.sourceVisitDate);
      }
      if (data.followUpSchedule) {
        updateData.followUpSchedule = data.followUpSchedule.map(date => Timestamp.fromDate(date));
      }
      if (data.approvedAt) {
        updateData.approvedAt = Timestamp.fromDate(data.approvedAt);
      }

      await this.db.collection("quotation_drafts").doc(id).update(updateData);

      const updated = await this.getQuotationDraft(id);
      if (!updated) throw new Error("Failed to retrieve updated quotation draft");
  ...
  async deleteQuotationDraft(id: string): Promise<void> {
    try {
      await this.db.collection("quotation_drafts").doc(id).delete();
    } catch (error) {
      console.error("Error deleting quotation draft:", error);
      throw error;
    }
  }

  // ===== ENHANCED QUOTATION MANAGEMENT METHODS =====

  async listQuotationsEnhanced(filters?: { 
    status?: string; 
    projectType?: string; 
    customerId?: string; 
    createdBy?: string;
    ...
    try {
      let query = this.db.collection("quotations_enhanced") as Query;
      ...
      if (filters?.approvedBy) {
        query = query.where("approvalWorkflow.approvedBy", "==", filters.approvedBy);
      }
      if (filters?.quotationNumber) {
        query = query.where("quotationNumber", "==", filters.quotationNumber);
      }
      ...
      return snapshot.docs.map(doc => {
        ...
        return {
          ...
          approvalWorkflow: {
            ...
            submittedAt: data.approvalWorkflow?.submittedAt?.toDate(),
            reviewedAt: data.approvalWorkflow?.reviewedAt?.toDate(),
            approvedAt: data.approvalWorkflow?.approvedAt?.toDate(),
            rejectedAt: data.approvalWorkflow?.rejectedAt?.toDate(),
          },
          customerInteractions: {
            ...data.customerInteractions,
            sentAt: data.customerInteractions?.sentAt?.toDate(),
            viewedAt: data.customerInteractions?.viewedAt?.toDate(),
            respondedAt: data.customerInteractions?.respondedAt?.toDate(),
            negotiationHistory: data.customerInteractions?.negotiationHistory?.map((item: any) => ({
              ...item,
              date: item.date?.toDate() || new Date(item.date),
            })) || [],
          },
          projectExecution: data.projectExecution ? {
            ...data.projectExecution,
            workOrderDate: data.projectExecution.workOrderDate?.toDate(),
            expectedStartDate: data.projectExecution.expectedStartDate?.toDate(),
            expectedCompletionDate: data.projectExecution.expectedCompletionDate?.toDate(),
            actualStartDate: data.projectExecution.actualStartDate?.toDate(),
            actualCompletionDate: data.projectExecution.actualCompletionDate?.toDate(),
          } : undefined,
        } as QuotationEnhanced;
      });
    } catch (error) {
      console.error("Error listing enhanced quotations:", error);
      return [];
    }
  }

  async getQuotationEnhanced(id: string): Promise<QuotationEnhanced | undefined> {
    try {
      const doc = await this.db.collection("quotations_enhanced").doc(id).get();
      if (!doc.exists) return undefined;
      ...
      console.error("Error getting enhanced quotation:", error);
      return undefined;
    }
  }

  async getQuotationByNumber(quotationNumber: string): Promise<QuotationEnhanced | undefined> {
    try {
      const snapshot = await this.db.collection("quotations_enhanced")
      ...
      return this.getQuotationEnhanced(snapshot.docs[0].id);
    } catch (error) {
  ...
  async getQuotationsByCustomer(customerId: string): Promise<QuotationEnhanced[]> {
    try {
      const snapshot = await this.db.collection("quotations_enhanced")
        .where("customerId", "==", customerId)
        .orderBy("createdAt", "desc")
      ...
      const quotations: QuotationEnhanced[] = [];
      for (const doc of snapshot.docs) {
        const quotation = await this.getQuotationEnhanced(doc.id);
        if (quotation) quotations.push(quotation);
      }

      return quotations;
    } catch (error) {
      console.error("Error getting quotations by customer:", error);
      return [];
    }
  }

  async createQuotationEnhanced(data: InsertQuotation): Promise<QuotationEnhanced> {
    try {
      const validatedData = insertQuotationEnhancedSchema.parse(data);
      const id = this.db.collection("quotations_enhanced").doc().id;
      const now = new Date();
      ...
      const quotationData = {
        ...
        approvedAt: validatedData.approvedAt ? Timestamp.fromDate(validatedData.approvedAt) : null,
        approvalWorkflow: {
          ...validatedData.approvalWorkflow,
          submittedAt: validatedData.approvalWorkflow.submittedAt ? Timestamp.fromDate(validatedData.approvalWorkflow.submittedAt) : undefined,
          reviewedAt: validatedData.approvalWorkflow.reviewedAt ? Timestamp.fromDate(validatedData.approvalWorkflow.reviewedAt) : undefined,
          approvedAt: validatedData.approvalWorkflow.approvedAt ? Timestamp.fromDate(validatedData.approvalWorkflow.approvedAt) : undefined,
          rejectedAt: validatedData.approvalWorkflow.rejectedAt ? Timestamp.fromDate(validatedData.approvalWorkflow.rejectedAt) : undefined,
        },
        customerInteractions: {
          ...validatedData.customerInteractions,
          sentAt: validatedData.customerInteractions.sentAt ? Timestamp.fromDate(validatedData.customerInteractions.sentAt) : undefined,
          viewedAt: validatedData.customerInteractions.viewedAt ? Timestamp.fromDate(validatedData.customerInteractions.viewedAt) : undefined,
          respondedAt: validatedData.customerInteractions.respondedAt ? Timestamp.fromDate(validatedData.customerInteractions.respondedAt) : undefined,
          negotiationHistory: validatedData.customerInteractions.negotiationHistory.map(item => ({
            ...item,
            date: Timestamp.fromDate(item.date),
          })),
        },
        projectExecution: validatedData.projectExecution ? {
          ...validatedData.projectExecution,
          workOrderDate: validatedData.projectExecution.workOrderDate ? Timestamp.fromDate(validatedData.projectExecution.workOrderDate) : undefined,
          expectedStartDate: validatedData.projectExecution.expectedStartDate ? Timestamp.fromDate(validatedData.projectExecution.expectedStartDate) : undefined,
          expectedCompletionDate: validatedData.projectExecution.expectedCompletionDate ? Timestamp.fromDate(validatedData.projectExecution.expectedCompletionDate) : undefined,
          actualStartDate: validatedData.projectExecution.actualStartDate ? Timestamp.fromDate(validatedData.projectExecution.actualStartDate) : undefined,
          actualCompletionDate: validatedData.projectExecution.actualCompletionDate ? Timestamp.fromDate(validatedData.projectExecution.actualCompletionDate) : undefined,
        } : undefined,
      };

      await this.db.collection("quotations_enhanced").doc(id).set(quotationData);
      ...
      console.error("Error creating enhanced quotation:", error);
      throw error;
    }
  }

  async updateQuotationEnhanced(id: string, data: Partial<InsertQuotation>): Promise<QuotationEnhanced> {
    try {
      const updateData: any = {
        ...data,
        updatedAt: Timestamp.fromDate(new Date()),
      ...
      if (data.approvalWorkflow) {
        updateData.approvalWorkflow = {
          ...data.approvalWorkflow,
          submittedAt: data.approvalWorkflow.submittedAt ? Timestamp.fromDate(data.approvalWorkflow.submittedAt) : undefined,
          reviewedAt: data.approvalWorkflow.reviewedAt ? Timestamp.fromDate(data.approvalWorkflow.reviewedAt) : undefined,
          approvedAt: data.approvalWorkflow.approvedAt ? Timestamp.fromDate(data.approvalWorkflow.approvedAt) : undefined,
          rejectedAt: data.approvalWorkflow.rejectedAt ? Timestamp.fromDate(data.approvalWorkflow.rejectedAt) : undefined,
        };
      }
      if (data.customerInteractions) {
        updateData.customerInteractions = {
          ...data.customerInteractions,
          sentAt: data.customerInteractions.sentAt ? Timestamp.fromDate(data.customerInteractions.sentAt) : undefined,
          viewedAt: data.customerInteractions.viewedAt ? Timestamp.fromDate(data.customerInteractions.viewedAt) : undefined,
          respondedAt: data.customerInteractions.respondedAt ? Timestamp.fromDate(data.customerInteractions.respondedAt) : undefined,
          negotiationHistory: data.customerInteractions.negotiationHistory?.map(item => ({
            ...item,
            date: Timestamp.fromDate(item.date),
          })) || [],
        };
      ...
      if (data.projectExecution) {
        updateData.projectExecution = {
          ...data.projectExecution,
          workOrderDate: data.projectExecution.workOrderDate ? Timestamp.fromDate(data.projectExecution.workOrderDate) : undefined,
          expectedStartDate: data.projectExecution.expectedStartDate ? Timestamp.fromDate(data.projectExecution.expectedStartDate) : undefined,
          expectedCompletionDate: data.projectExecution.expectedCompletionDate ? Timestamp.fromDate(data.projectExecution.expectedCompletionDate) : undefined,
          actualStartDate: data.projectExecution.actualStartDate ? Timestamp.fromDate(data.projectExecution.actualStartDate) : undefined,
          actualCompletionDate: data.projectExecution.actualCompletionDate ? Timestamp.fromDate(data.projectExecution.actualCompletionDate) : undefined,
        };
      }

      await this.db.collection("quotations_enhanced").doc(id).update(updateData);

      const updated = await this.getQuotationEnhanced(id);
      if (!updated) throw new Error("Failed to retrieve updated enhanced quotation");

      return updated;
    } catch (error) {
      console.error("Error updating enhanced quotation:", error);
      throw error;
    }
  }

  async deleteQuotationEnhanced(id: string): Promise<void> {
    try {
      await this.db.collection("quotations_enhanced").doc(id).delete();
    } catch (error) {
      console.error("Error deleting enhanced quotation:", error);
      throw error;
    }
  }

  // ===== QUOTATION WORKFLOW MANAGEMENT METHODS =====

  async promoteQuotationDraftToFinal(draftId: string, quotationNumber: string): Promise<QuotationEnhanced> {
    try {
      const draft = await this.getQuotationDraft(draftId);
      if (!draft) throw new Error("Draft quotation not found");

      // Create enhanced quotation from draft
      const enhancedData: InsertQuotation = {
        ...draft,
        quotationNumber,
        version: 1,
        previousVersions: [],
        approvalWorkflow: {},
        customerInteractions: { negotiationHistory: [] },
      };

      const enhanced = await this.createQuotationEnhanced(enhancedData);

      // Update draft status
      await this.updateQuotationDraft(draftId, { 
        status: "promoted",
        quotationNumber,
      });
      ...
      console.error("Error promoting quotation draft to final:", error);
      throw error;
    }
  }

  async submitQuotationForApproval(id: string, submittedBy: string): Promise<QuotationEnhanced> {
    try {
      const now = new Date();
      const updateData = {
      ...
      await this.updateQuotationEnhanced(id, updateData);
      
      const updated = await this.getQuotationEnhanced(id);
      if (!updated) throw new Error("Failed to retrieve submitted quotation");
  ...
  async approveQuotation(id: string, approvedBy: string): Promise<QuotationEnhanced> {
    try {
      const now = new Date();
      const updateData = {
        status: "approved",
      ...
      const updated = await this.getQuotationEnhanced(id);
      if (!updated) throw new Error("Failed to retrieve approved quotation");
  ...
  async rejectQuotation(id: string, rejectedBy: string, reason: string): Promise<QuotationEnhanced> {
    try {
      const now = new Date();
      const updateData = {
        status: "rejected",
      ...
      const updated = await this.getQuotationEnhanced(id);
      if (!updated) throw new Error("Failed to retrieve rejected quotation");
  ...
  async sendQuotationToCustomer(id: string, sentBy: string): Promise<QuotationEnhanced> {
    try {
      const now = new Date();
      const updateData = {
      ...
      const updated = await this.getQuotationEnhanced(id);
      if (!updated) throw new Error("Failed to retrieve sent quotation");
  ...
  // ===== QUOTATION DOCUMENT MANAGEMENT METHODS =====

  async listQuotationDocuments(quotationId: string): Promise<QuotationDocument[]> {
    try {
      const snapshot = await this.db.collection("quotation_documents")
      ...
      console.error("Error listing quotation documents:", error);
      return [];
    }
  }

  async getQuotationDocument(id: string): Promise<QuotationDocument | undefined> {
    try {
      const doc = await this.db.collection("quotation_documents").doc(id).get();
      if (!doc.exists) return undefined;
      ...
      console.error("Error getting quotation document:", error);
      return undefined;
    }
  }

  async createQuotationDocument(data: z.infer<typeof quotationDocumentSchema>): Promise<QuotationDocument> {
    try {
      const validatedData = quotationDocumentSchema.parse(data);
      const id = this.db.collection("quotation_documents").doc().id;

      const documentData = {
        ...validatedData,
        uploadedAt: Timestamp.fromDate(validatedData.uploadedAt),
      };

      await this.db.collection("quotation_documents").doc(id).set(documentData);
  ...
  async updateQuotationDocument(id: string, data: Partial<z.infer<typeof quotationDocumentSchema>>): Promise<QuotationDocument> {
    try {
      const updateData: any = { ...data };

      if (data.uploadedAt) {
        updateData.uploadedAt = Timestamp.fromDate(data.uploadedAt);
      }

      await this.db.collection("quotation_documents").doc(id).update(updateData);

      const updated = await this.getQuotationDocument(id);
      if (!updated) throw new Error("Failed to retrieve updated quotation document");

      return updated;
    } catch (error) {
      console.error("Error updating quotation document:", error);
      throw error;
    }
  }

  async deleteQuotationDocument(id: string): Promise<void> {
    try {
      await this.db.collection("quotation_documents").doc(id).update({
        isDeleted: true,
      });
    } catch (error) {
  ...
  // ===== QUOTATION COMMUNICATION TRACKING METHODS =====

  async listQuotationCommunications(quotationId: string): Promise<QuotationCommunication[]> {
    try {
      const snapshot = await this.db.collection("quotation_communications")
        .where("quotationId", "==", quotationId)
        .orderBy("sentAt", "desc")
        .get();
      ...
      return snapshot.docs.map(doc => {
        ...
        } as QuotationCommunication;
      });
    } catch (error) {
      console.error("Error listing quotation communications:", error);
      return [];
    }
  }

  async getQuotationCommunication(id: string): Promise<QuotationCommunication | undefined> {
    try {
      const doc = await this.db.collection("quotation_communications").doc(id).get();
      if (!doc.exists) return undefined;
      ...
      console.error("Error getting quotation communication:", error);
      return undefined;
    }
  }

  async createQuotationCommunication(data: z.infer<typeof quotationCommunicationSchema>): Promise<QuotationCommunication> {
    try {
      const validatedData = quotationCommunicationSchema.parse(data);
      const id = this.db.collection("quotation_communications").doc().id;

      const communicationData = {
        ...validatedData,
        sentAt: Timestamp.fromDate(validatedData.sentAt),
      };

      await this.db.collection("quotation_communications").doc(id).set(communicationData);
      ...
      console.error("Error creating quotation communication:", error);
      throw error;
    }
  }

  async updateQuotationCommunication(id: string, data: Partial<z.infer<typeof quotationCommunicationSchema>>): Promise<QuotationCommunication> {
    try {
      const updateData: any = { ...data };

      if (data.sentAt) {
        updateData.sentAt = Timestamp.fromDate(data.sentAt);
      }

      await this.db.collection("quotation_communications").doc(id).update(updateData);

      const updated = await this.getQuotationCommunication(id);
      if (!updated) throw new Error("Failed to retrieve updated quotation communication");

      return updated;
    } catch (error) {
      console.error("Error updating quotation communication:", error);
      throw error;
    }
  }

  // ===== PRICING HISTORY MANAGEMENT METHODS =====

  async listPricingHistory(quotationId: string): Promise<PricingHistory[]> {
    try {
      const snapshot = await this.db.collection("pricing_history")
        .where("quotationId", "==", quotationId)
      ...
      console.error("Error listing pricing history:", error);
      return [];
    }
  }

  async getPricingHistory(id: string): Promise<PricingHistory | undefined> {
    try {
      const doc = await this.db.collection("pricing_history").doc(id).get();
      if (!doc.exists) return undefined;
  ...
  async createPricingHistory(data: z.infer<typeof pricingHistorySchema>): Promise<PricingHistory> {
    try {
      const validatedData = pricingHistorySchema.parse(data);
      const id = this.db.collection("pricing_history").doc().id;

      const historyData = {
        ...validatedData,
        changedAt: Timestamp.fromDate(validatedData.changedAt),
      };

      await this.db.collection("pricing_history").doc(id).set(historyData);
  ...
  // ===== QUOTATION TEMPLATE MANAGEMENT METHODS =====

  async getQuotationTemplate(quotationId: string): Promise<QuotationTemplate | undefined> {
    try {
      const snapshot = await this.db.collection("quotation_templates")
        .where("quotationId", "==", quotationId)
        .limit(1)
        .get();
  ...
  async createQuotationTemplate(data: z.infer<typeof quotationTemplateSchema>): Promise<QuotationTemplate> {
    try {
      const validatedData = quotationTemplateSchema.parse(data);
      const id = this.db.collection("quotation_templates").doc().id;

      await this.db.collection("quotation_templates").doc(id).set(validatedData);
  ...
  async updateQuotationTemplate(id: string, data: Partial<z.infer<typeof quotationTemplateSchema>>): Promise<QuotationTemplate> {
    try {
      await this.db.collection("quotation_templates").doc(id).update(data);

      const updated = await this.db.collection("quotation_templates").doc(id).get();
      if (!updated.exists) throw new Error("Failed to retrieve updated quotation template");
  ...
  // ===== QUOTATION ANALYTICS AND DATA COMPLETENESS METHODS =====

  async analyzeQuotationDataCompleteness(siteVisitId: string): Promise<DataCompletenessReport> {
    try {
      // This would integrate with the QuotationDataAnalyzer service
      ...
      console.error("Error analyzing quotation data completeness:", error);
      return {
        projectTypes: [],
        overallCompleteness: 0
      };
  ...
  async getQuotationAnalytics(filters?: {
    startDate?: Date;
    endDate?: Date;
    projectType?: string;
    status?: string;
    ...
    try {
      ...
      const totalValue = quotations.reduce((sum, q) => sum + (q.pricing?.customerPayment || 0), 0);
      const averageValue = totalQuotations > 0 ? totalValue / totalQuotations : 0;
      
      // Calculate conversion rate (approved/sent vs total)
      const convertedCount = quotations.filter(q => 
        q.status === "approved" || q.status === "sent_to_customer" || q.status === "work_order_issued"
      ).length;
      const conversionRate = totalQuotations > 0 ? convertedCount / totalQuotations : 0;
      ...
      quotations.forEach(q => {
        projectTypeBreakdown[q.projectType] = (projectTypeBreakdown[q.projectType] || 0) + 1;
      });
  ...
  // ===== QUOTATION NUMBER GENERATION METHOD =====

  async generateQuotationNumber(projectType: string, customerId: string): Promise<string> {
    try {
      ...
      // Get sequential number for this month and type
      const counterDoc = this.db.collection("counters").doc(`quotation_${typePrefix}_${year}_${month}`);
      ...
      try {
        const result = await this.db.runTransaction(async (transaction) => {
          ...
          if (doc.exists) {
            counter = doc.data()!.value + 1;
          }
          
          transaction.set(counterDoc, { value: counter });
          return counter;
        });
        counter = result;
      } catch (error) {
        console.warn("Transaction failed, using fallback counter:", error);
        // Fallback: query existing quotations for this month
      ...
      return `${typePrefix}-${year}${month}-${sequentialNumber}`;
    } catch (error) {
